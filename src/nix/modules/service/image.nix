  ...
  options = {
    ...
    image.name = mkOption {
      ...
      description = ''
        ...
        **Note** that if you set `services.<name>.image.tarball` to an
        arbitrary Docker image tarball (and not, say, a derivation produced by
        one of the [`dockerTools`](https://nixos.org/manual/nixpkgs/stable/#sec-pkgs-dockerTools)
        builder functions), then you **must** set `services.<name>.image.name`
        to the name of the image in the tarball.  Otherwise, Arion will not
        be able to properly identify the image in the generated Docker Compose
        configuration file.
      '';
    };
    image.tag = mkOption {
      ...
      description = ''
        ...
        **Note** that if you set `services.<name>.image.tarball` to an
        arbitrary Docker image tarball (and not, say, a derivation produced by
        one of the [`dockerTools`](https://nixos.org/manual/nixpkgs/stable/#sec-pkgs-dockerTools)
        builder functions), then you **must** set `services.<name>.image.tag`
    ...
    image.tarball = mkOption {
      ...
      description = ''
        ...
        **Note** that using this option causes Arion to ignore most other
        ...
        used when the provided `services.<name>.image.tarball` is not a
        derivation with the attributes `imageName` and `imageTag`.
      '';
      default = builtImage;
